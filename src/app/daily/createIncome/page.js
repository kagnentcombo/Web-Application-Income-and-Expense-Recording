...
export default function page() {
  ...
  const handleCreateIncome = async () => {
    try {
      const response = await axios.post('http://localhost:3001/createIncome', incomeData);
	...
	return (
      ...
      />
        ...
        variant="outlined"
        name="amountOfMoney"
        value={incomeData.amountOfMoney}
        onChange={handleInputChange}
      />
						</Item>
						<Item>
        ...
        <DatePicker
          ...
          onChange={(date) => setIncomeData((prevData) => ({ ...prevData, date }))}
